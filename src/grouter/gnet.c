...
int GNETInit(int *ghandler, char *config_dir, char *rname, simplequeue_t *sq)
{
	...
	GNETInitInterfaces();
 	...
 	//create new thread to run GNETHandler
...
void *GNETHandler(void *outq)
{
	...
	while (1)
	{
		//dequeues a packet from the output queue. This statement blocks if there is no packet in the output queue.
		...
		//finds the output interface structure that corresponds to the interface number specified in the packet
		//pkt is “super” packet that includes the IP packet in wire format and related meta information
